...
async function main() {
  ...
  // Usuário admin: defina SEED_ADMIN_PASSWORD no .env / .env.local (nunca commite senhas).
  const adminEmail = process.env.SEED_ADMIN_EMAIL || 'admin@shopmy.com'
  const adminPassword = process.env.SEED_ADMIN_PASSWORD
  if (!adminPassword || adminPassword.length < 8) {
    throw new Error(
      'Defina SEED_ADMIN_PASSWORD no ambiente (mínimo 8 caracteres) antes de npm run db:seed.'
    )
  }
